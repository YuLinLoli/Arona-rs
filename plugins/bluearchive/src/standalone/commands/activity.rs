...
async fn activity_message(
    ...
    match crate::image::activity::render_async(pair.clone(), server).await {
        ...
        Err(err) => {
            arona::runtime::log::warning(format!("活动图片生成失败, 回退文本: {err}"));
    ...
    match data::activity::fetch(server).await {
        Ok(pair) => {
            ...
            match crate::image::activity::render_async(pair, server).await {
                Ok(file) => arona::runtime::log::info(format!(
                    "{}本地活动图片已刷新: {}",
                    server.server_name(),
                    file.display()
                )),
                Err(err) => arona::runtime::log::warning(format!(
                    "刷新{}本地活动图片失败: {err}",
                    server.server_name()
                )),
            }
...
pub fn enable_image_refresh_job() {
    ...
    arona::runtime::log::info("本地活动图片刷新任务已启用: 每天 0 点");
}
...
mod tests {
    ...
    async fn image_refresh_job_registered_at_midnight() {
        ...
        arona::quartz::remove("AronaActivityImageRefreshDaily");
    }
}
