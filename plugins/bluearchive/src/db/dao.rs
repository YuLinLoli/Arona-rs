...
pub struct ChatMessageRow {
    ...
    pub group: i64,
    pub qq: i64,
    /// 机器人自己发的那条：只有连出站一起记，才还原得了"用户引用了机器人的回复"
    pub from_bot: bool,
    pub name: String,
    /// 消息时间（秒）
    pub time: i64,
    pub text: String,
    /// 图片的原始链接（入站）或本地路径（机器人生成的图），空表示没有图。
    /// 原始链接会过期，取用时得先探一次
    pub image: String,
}

pub fn remember_chat_message(row: &ChatMessageRow) {
    let from_bot = if row.from_bot { 1 } else { 0 };
    let _ = super::query(|conn| {
        conn.execute(
            "INSERT OR REPLACE INTO chat_message
             (message_id, grp, qq, from_bot, name, time, text, image)
...
pub fn find_chat_message(message_id: i64) -> Option<ChatMessageRow> {
    super::query(|conn| {
        conn.query_row(
            "SELECT message_id, grp, qq, from_bot, name, time, text, image
...
/// 删掉 `time < cutoff` 的记录，返回删掉的条数
pub fn forget_chat_messages(cutoff: i64) -> usize {
    super::query(|conn| conn.execute("DELETE FROM chat_message WHERE time < ?1", params![cutoff]))
        .unwrap_or(0)
}
