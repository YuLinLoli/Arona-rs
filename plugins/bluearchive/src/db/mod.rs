...
-- 聊天记录不在这里了：引用还原所需的记账已由框架统一维护（data/arona/chatlog.db）。
...
pub fn start() -> bool {
    ...
    let conn = match Connection::open(&path) {
        Ok(conn) => conn,
        Err(err) => {
            arona::runtime::log::error(format!("database open failed: {err}"));
            return false;
        }
    };
    ...
    if let Err(err) = conn.execute_batch(SCHEMA) {
        arona::runtime::log::error(format!("database schema init failed: {err}"));
        return false;
    }
...
pub fn query<T>(f: impl FnOnce(&Connection) -> rusqlite::Result<T>) -> Option<T> {
    ...
    match f(conn) {
        Ok(value) => Some(value),
        Err(err) => {
            arona::runtime::log::error(format!("database query failed: {err}"));
            None
        }
    }
}
