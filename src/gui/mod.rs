...
impl AronaGui {
    ...
    fn logs(&mut self, ui: &mut egui::Ui, _ctx: &egui::Context) {
        ...
        let lines = &self.log_lines[start..];
        egui::ScrollArea::vertical()
            .auto_shrink([false, false])
            ...
            .show(ui, |ui| {
                ...
                for line in lines {
                    if errors_only && !is_alert_line(line) {
                        continue;
                    }
                    ...
                    shown += 1;
                    let art = is_ascii_art_line(&line.text);
                    ui.horizontal_wrapped(|ui| {
                        ui.spacing_mut().item_spacing.x = 6.0;
                        ui.weak(format!("[{}]", line.time));
                        let color = log_line_color(&line.text, line.stderr, ui.visuals().dark_mode);
                        let mut label =
                            egui::Label::new(egui::RichText::new(&line.text).monospace().color(color));
                        if art {
...
/// 渲染后端候选顺序：默认先试 glow(OpenGL)，再依次试 wgpu 的几种配置。
/// 可用 `--renderer=glow` / `--renderer=wgpu` 只试其中一族。
fn renderer_candidates(args: &[String]) -> Vec<RendererAttempt> {
    let auto = || {
        let mut attempts = Vec::new();
        // 没有硬件 OpenGL 时 glow 一定失败（而且可能直接终止进程），直接跳过。
        // 只有走软件 OpenGL(softgl，Mesa llvmpipe) 时才必须保留 glow —— 那正是渲染器本身。
        if crate::runtime::softgl::requested(args) || has_hardware_opengl() {
            attempts.push(RendererAttempt::glow());
        } else if crate::runtime::softgl::dir().is_some() {
            crate::runtime::log::warning(
                "系统未安装 OpenGL 驱动(ICD)，跳过 glow 硬件后端；已检测到随程序附带的软件 OpenGL，必要时会自动启用",
            );
        } else {
            crate::runtime::log::warning(
                "系统未安装 OpenGL 驱动(ICD)，跳过 glow 后端；无显卡驱动/无 DX12 的机器可运行 scripts/fetch-softgl.ps1 获取软件 OpenGL",
            );
        }
        attempts.push(RendererAttempt::wgpu("wgpu(DX12/Vulkan)", WgpuFlavor::Primary));
        attempts.push(RendererAttempt::wgpu("wgpu(全部后端)", WgpuFlavor::AllBackends));
        attempts.push(RendererAttempt::wgpu("wgpu(DX12+FXC)", WgpuFlavor::Fxc));
        attempts
    };
    match crate::find_arg(args, "--renderer=").as_deref() {
        Some("wgpu") => vec![
        ...
        Some("glow") => vec![RendererAttempt::glow()],
        Some(other) => {
            crate::runtime::console::eprint_safe(&format!(
                "[Arona] 未知渲染后端 {other}（可用 glow / wgpu），改用自动选择"
            ));
            auto()
        }
        None => auto(),
...
pub fn run(args: Vec<String>) -> Result<(), String> {
    ...
    // 依次尝试各个渲染后端：glow 失败(如只有 OpenGL 1.1)时自动改用 wgpu(DX12/WARP)
    let mut errors: Vec<String> = Vec::new();
    let mut outcome: Result<(), String> = Err("没有可用的渲染后端".to_string());
    for attempt in renderer_candidates(&args) {
        let (tx, rx) = channel();
        let creator = app_creator(handle.clone(), tx, rx);
        ...
        crate::runtime::log::info(format!(
            "管理面板启动中（渲染后端: {}）{}",
            attempt.name,
            if attempt.flavor == WgpuFlavor::Primary {
                ""
            } else {
                " [备用方案]"
            }
        ));
        match eframe::run_native("Arona 管理面板", options, creator) {
            Ok(()) => {
                outcome = Ok(());
                break;
            }
