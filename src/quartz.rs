...
/// 每日活动推送任务：小时变化时重建（供 arona.yml 热重载调用）
pub fn reschedule_daily_notify(hour: i64) {
    if !exists("StandaloneActivityNotify") {
        return;
    }
    remove("StandaloneActivityNotify");
    crate::activity::notify::enable_daily_job(hour.clamp(0, 23) as u32);
}
