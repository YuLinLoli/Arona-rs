...
fn setup() -> (
    Arc<crate::runtime::dispatcher::SimpleCommandDispatcher>,
    Arc<CaptureSender>,
    i64,
) {
    crate::runtime::paths::prepare_standalone_root();
    crate::runtime::services::set_data_root(crate::runtime::paths::data_root());
    crate::runtime::config::set_bot_id(10000);
    crate::runtime::config::set_end_with_sensei("老师".to_string());
...
async fn trainer_cloud_image_and_numeric_reply() {
    ...
    let suggestion = messages
        ...
        .find_map(|message| match message.segments.first() {
            Some(MessageSegment::Text(text)) if text.contains("最接近的有") => Some(text.clone()),
            _ => None,
        })
        .expect("模糊关键词未返回建议列表");
    println!("[模糊建议]\n{suggestion}");
