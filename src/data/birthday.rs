...
pub async fn upcoming_birthday_activities(server: ServerLocale) -> Vec<Activity> {
    let list = match fetch_birthdays().await {
        Ok(list) => list,
        Err(err) => {
            crate::runtime::log::warning(format!("学生生日数据获取失败: {err}"));
            return Vec::new();
        }
    };
...
mod tests {
    ...
    fn window_covers_next_week_only() {
        ...
        let list = vec![
            Birthday { name: "今天".into(), month: 9, day: 11 },
            Birthday { name: "明天".into(), month: 9, day: 12 },
            Birthday { name: "第六天".into(), month: 9, day: 17 },
            Birthday { name: "第七天".into(), month: 9, day: 18 },
            Birthday { name: "下个月".into(), month: 10, day: 1 },
        ];
        ...
        assert!(!names.contains(&"今天的生日".to_string()), "当天生日不入列表");
        assert!(!names.contains(&"第七天的生日".to_string()), "第 7 天超出窗口");
    ...
    async fn fetch_real_birthdays() {
        ...
        for server in ServerLocale::ALL {
            ...
            println!("{} 未来一周生日 {} 条", server.server_name(), activities.len());
            for activity in &activities {
