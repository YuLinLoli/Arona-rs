...
;
;  本仓库只交付框架本体：OneBot 连接、群授权、命令分发、插件生命周期与管理面板。
;  安装包不含任何功能插件——功能插件是编译成 dll 的独立 crate，由用户放进安装目录的
;  plugins\ 下，框架启动时自动扫描装载（插件开发规范见 PLUGIN_DEVELOPMENT.md）。
;
;  安装/更新行为：
...
; 随包再放一份「安装说明.txt」，就是向导第一页那份文案：装完打开就能看到能配什么、
; 缺什么（玩法要另外装功能插件），不必去翻仓库
...
;      功能插件各自再占一层子目录（config/<插件>/、data/<插件>/），由插件自己生成模板
