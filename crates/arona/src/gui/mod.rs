...
fn accent_rgb(dark: bool) -> (u8, u8, u8) {
    if dark { (96, 196, 240) } else { (0, 128, 186) }
}

/// 主强调色：黑夜用亮天蓝，白天用深蓝，两者在各自背景上都有足够对比度
fn accent(dark: bool) -> egui::Color32 {
    let (r, g, b) = accent_rgb(dark);
    egui::Color32::from_rgb(r, g, b)
}

/// 半透明强调色（选中底、淡色块）
fn accent_alpha(dark: bool, alpha: u8) -> egui::Color32 {
    let (r, g, b) = accent_rgb(dark);
    egui::Color32::from_rgba_unmultiplied(r, g, b, alpha)
}

/// `Stroke::new` 的宽度参数是 `impl Into<f32>`，直接写字面量会让 rustc 在数值类型上兜底报警，
/// 这个包装把宽度钉成 f32。
fn line(width: f32, color: egui::Color32) -> egui::Stroke {
    egui::Stroke::new(width, color)
}

/// 把 egui 自带的黑/白主题改成 Arona 配色：蓝灰底 + 圆角 + 统一的间距节奏。
///
/// 只覆盖颜色与圆角，不动 `Visuals::dark()/light()` 的其它行为（如 striped、
/// text_cursor），这样深浅两套都跟着官方主题走，将来升级 egui 不容易跑偏。
fn apply_palette(ctx: &egui::Context, dark: bool) {
    ...
    // 背景：深色用带一点蓝的深灰，浅色用接近白的蓝白
    let (panel, card, field, faint, stroke) = if dark {
        (
            egui::Color32::from_rgb(17, 21, 27),
            egui::Color32::from_rgb(26, 32, 41),
            egui::Color32::from_rgb(12, 15, 20),
            egui::Color32::from_rgb(23, 28, 35),
            egui::Color32::from_rgb(48, 57, 71),
        )
    } else {
        (
            egui::Color32::from_rgb(243, 247, 251),
            egui::Color32::from_rgb(255, 255, 255),
            egui::Color32::from_rgb(252, 253, 255),
            egui::Color32::from_rgb(237, 243, 250),
            egui::Color32::from_rgb(206, 217, 229),
        )
    };
    ...
    visuals.hyperlink_color = accent(dark);
    visuals.window_stroke = line(1.0, stroke);
    ...
    visuals.selection.bg_fill = accent_alpha(dark, if dark { 58 } else { 42 });
    visuals.selection.stroke = line(1.0, accent(dark));
    ...
    for state in [
        ...
        // 复选框只有 interact_size.y（14pt）见方，圆角再大就被拉成圆形，看着像单选框
        state.corner_radius = egui::CornerRadius::same(5);
    }
    let text = if dark {
        egui::Color32::from_rgb(226, 234, 244)
    } else {
        egui::Color32::from_rgb(28, 38, 50)
    };
    widgets.noninteractive.bg_fill = card;
    widgets.noninteractive.fg_stroke = line(1.0, text);
    widgets.inactive.bg_fill = card;
    widgets.inactive.weak_bg_fill = if dark {
        egui::Color32::from_rgb(37, 45, 57)
    } else {
        egui::Color32::from_rgb(237, 243, 250)
    };
    widgets.inactive.bg_stroke = line(1.0, stroke);
    // 勾号（复选框）和单选点都用 fg_stroke 画，1pt 在 14pt 的小方框里几乎看不清；
    // 这里只加宽度不改颜色，文字取的是同一条 stroke 的 color，不受影响。
    widgets.inactive.fg_stroke = line(1.75, text);
    widgets.hovered.bg_fill = accent_alpha(dark, if dark { 46 } else { 30 });
    widgets.hovered.weak_bg_fill = if dark {
        egui::Color32::from_rgb(46, 60, 78)
    } else {
        egui::Color32::from_rgb(222, 236, 248)
    };
    widgets.hovered.bg_stroke = line(1.0, accent(dark));
    widgets.hovered.fg_stroke = line(1.75, text);
    // 注意：egui 的 `Visuals::strong_text_color()` 取的就是 `widgets.active.fg_stroke.color`，
    // 这里若填「压在强调色上的反色」，全程序所有 `RichText::strong()` 文字都会跟着变深色。
    widgets.active.bg_fill = accent_alpha(dark, if dark { 96 } else { 56 });
    widgets.active.weak_bg_fill = accent_alpha(dark, if dark { 96 } else { 56 });
    widgets.active.bg_stroke = line(1.0, accent(dark));
    widgets.active.fg_stroke = line(1.75, text);
    widgets.open.fg_stroke = line(1.75, text);
    widgets.open.bg_stroke = line(1.0, accent(dark));
    ...
    ctx.style_mut(|style| {
        ...
        for (name, size) in [
            ...
            if let Some(font) = style.text_styles.get_mut(&name) {
                font.size = size;
            }
        }
    });
    ...
    egui::Frame::NONE
        ...
        .stroke(line(
            ...
            if dark {
                ...
                egui::Color32::from_rgb(224, 232, 241)
            },
        ))
...
fn section_title(ui: &mut egui::Ui, text: &str) {
    ui.add(egui::Label::new(egui::RichText::new(text).strong().size(15.0)).selectable(false));
}

/// 整行宽度的卡片：把一段内容包成一块圆角面板，用于把长页面切成可读的分块
fn card_row<T>(
    ui: &mut egui::Ui,
    dark: bool,
    add: impl FnOnce(&mut egui::Ui) -> T,
) -> egui::InnerResponse<T> {
    card(dark).show(ui, |ui| {
        ui.set_width(ui.available_width());
        add(ui)
    })
}

/// 页签按钮：选中时用主色实心，未选中时只有一圈淡描边
fn tab_button(ui: &mut egui::Ui, selected: bool, label: &str) -> bool {
    ...
    ui.add(
        egui::Button::new(text)
            ...
            .stroke(line(
                if selected { 0.0 } else { 1.0 },
                if dark {
                    egui::Color32::from_rgb(48, 57, 71)
                } else {
                    egui::Color32::from_rgb(214, 224, 236)
                },
            ))
...
/// 外观下拉（返回是否改了选择）
fn theme_combo(ui: &mut egui::Ui, theme: &mut ThemePref) -> bool {
    let before = *theme;
    egui::ComboBox::from_id_salt("theme")
        ...
        .show_ui(ui, |ui| {
            for option in [ThemePref::System, ThemePref::Light, ThemePref::Dark] {
                ui.selectable_value(theme, option, option.label());
            }
        });
    *theme != before
}

/// 右上角运行状态：绿点=已有活动连接，灰点=还没连上任何 OneBot 实现
fn status_chip(ui: &mut egui::Ui) {
    let dark = ui.visuals().dark_mode;
    let connections = crate::onebot::connection::global_registry()
        .map(|registry| registry.count())
        .unwrap_or(0);
    let dot = if connections > 0 {
        ok_color(dark)
    } else {
        if dark {
    ...
    ui.horizontal(|ui| {
        ...
        ui.add(egui::Label::new(egui::RichText::new("●").size(11.0).color(dot)).selectable(false));
        ui.add(
            egui::Label::new(
...
struct AronaGui {
    ...
    /// 已下发的配色对应的是深色还是浅色（跟随系统时系统换主题要重刷）
    palette_applied: Option<bool>,
    /// 品牌图（assets/arona.png），标题栏与空状态用
...
impl AronaGui {
    ...
    fn save_onebot(&mut self, ctx: &egui::Context) {
        ...
        self.spawn(
            ctx,
            async move { Msg::Note(admin::save_and_reload(&config)) },
        );
    }

    /// 载入品牌图（只解码一次，之后复用 egui 的纹理）
    fn ensure_icon(&mut self, ctx: &egui::Context) {
        if self.icon.is_some() {
            return;
        }
        const PNG: &[u8] = include_bytes!("../../../../assets/arona.png");
        let Ok(rgba) = image::load_from_memory(PNG).map(|image| image.to_rgba8()) else {
            return;
        };
        let size = [rgba.width() as usize, rgba.height() as usize];
        let image = egui::ColorImage::from_rgba_unmultiplied(size, rgba.as_raw());
        self.icon = Some(ctx.load_texture("arona-icon", image, egui::TextureOptions::LINEAR));
    }

    /// 空状态：居中放一张品牌图 + 两句说明，比孤零零一行小箭头友好
    ///
    /// 用 `vertical_centered` 而不是 `centered_and_justified`：后者沿主方向（这里就是纵向）
    /// 也做 justify，图标会被拉高、两句说明直接被顶到面板外面看不见。
    /// 折行宽度靠 `set_width` 限定，否则说明文字按内容收缩，长句子会溢出右边界。
    fn empty_state(&self, ui: &mut egui::Ui, title: &str, hint: &str) {
        let width = ui.available_width().min(460.0);
        ui.vertical_centered(|ui| {
            ui.add_space((ui.available_height() * 0.26).max(24.0));
            if let Some(icon) = &self.icon {
                ui.add(
                    egui::Image::from_texture(icon)
                        .max_size(egui::vec2(104.0, 104.0))
            ...
            ui.set_width(width);
            ui.add(
                egui::Label::new(egui::RichText::new(title).strong().size(16.0))
                    .selectable(false)
                    .wrap_mode(egui::TextWrapMode::Wrap),
            );
            ui.add(
                egui::Label::new(egui::RichText::new(hint).weak())
                    .selectable(false)
                    .wrap_mode(egui::TextWrapMode::Wrap),
            );
        });
    ...
    fn header(&mut self, ui: &mut egui::Ui) {
        let dark = ui.visuals().dark_mode;
        // 第一行：品牌 + 运行状态 + 外观；第二行：页签。
        // 分成两行是因为状态文字较长，和四个页签挤在同一行时会把右侧控件顶出窗口。
        ui.horizontal(|ui| {
            if let Some(icon) = &self.icon {
                ui.add(
                    egui::Image::from_texture(icon)
                        .max_size(egui::vec2(34.0, 34.0))
                        .sense(egui::Sense::hover()),
                );
            }
            ui.vertical(|ui| {
                ...
                ui.add(
                    egui::Label::new(egui::RichText::new("管理面板").size(11.0).weak())
                        .selectable(false),
                );
            });
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                let mut theme = self.theme;
                if theme_combo(ui, &mut theme) {
                    self.theme = theme;
                    save_theme(theme);
                }
                ui.add_space(4.0);
        ...
        ui.horizontal(|ui| {
            for (tab, label) in [
                ...
                if tab_button(ui, self.tab == tab, label) {
                    self.tab = tab;
                }
                ui.add_space(3.0);
            }
    ...
    fn group_list(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        if let Some(err) = self.groups_error.clone() {
            ui.colored_label(error_color(dark), err);
        }
        ui.add_space(4.0);
        ...
        ui.add(
            egui::Label::new(
                egui::RichText::new(if filter.is_empty() {
                    format!("共 {} 个群", groups.len())
                } else {
                    format!("匹配 {matched} / {} 个群", groups.len())
                })
                .size(12.0)
                .weak(),
            )
            ...
            .show(ui, |ui| {
                ...
                for info in &groups {
                    ...
                    let label = egui::RichText::new(format!(
                        "{mark} {}\n　{} · {} 人{tag}",
                        info.name, info.group_id, info.member_count
                    ))
                    .size(13.5);
                    if ui
    ...
    fn group_detail(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        let dark = ui.visuals().dark_mode;
        ui.add(
            egui::Label::new(egui::RichText::new(name.as_str()).strong().size(20.0))
                .selectable(false),
        );
        ui.add(
            egui::Label::new(
                egui::RichText::new(format!("群号 {group_id}"))
                    .size(12.0)
                    .weak(),
            )
            .selectable(false),
        ...
        card_row(ui, dark, |ui| {
            ui.horizontal(|ui| {
                ...
                if let Some(item) = &info {
                    if item.local_only {
                        ui.colored_label(warn_color(dark), "(不在 OneBot 群列表中)");
                    }
                }
            });
        });
                        ...
                        .show(ui, |ui| {
                            ...
                            for index in indices {
                                ...
                                if ui.checkbox(&mut group_black, "").changed() {
                                    let result =
                                        admin::set_group_blacklist(group_id, user_id, group_black);
                                    self.note(result);
    ...
    fn connections(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        let Some(config) = self.onebot.clone() else {
            self.empty_state(
                ui,
                "还没有读取配置",
                "点上方「读取 onebot.yml」载入连接列表",
            );
            return;
        };
        ui.add_space(6.0);
        card_row(ui, dark, |ui| {
            section_title(ui, "连接列表");
            ui.add(
                egui::Label::new(
                    egui::RichText::new("同一类型可以添加多个连接")
                        .size(12.0)
                        .weak(),
                )
                .selectable(false),
            ...
            egui::Grid::new("connection_grid")
                ...
                .show(ui, |ui| {
                    ...
                    for info in &list {
                        ...
                        // 勾本身在小尺寸下几乎看不清，状态直接写成文字
                        let state_text = if enable { "已启用" } else { "未启用" };
                        if ui.checkbox(&mut enable, state_text).changed() {
        ...
        card_row(ui, dark, |ui| {
            section_title(ui, "发送设置");
            ui.add(
                egui::Label::new(
                    egui::RichText::new("勾选后立即生效并写入 onebot.yml，无需点「保存并热重载」")
                        .size(12.0)
                        .weak(),
                )
                .selectable(false),
...
impl eframe::App for AronaGui {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Arona 配色按「实际生效」的深浅下发：跟随系统时系统换主题也能跟上
        let dark = ctx.style().visuals.dark_mode;
        if self.palette_applied != Some(dark) {
            self.palette_applied = Some(dark);
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    if let Some(toast) = &self.toast {
                        ...
                        ui.add(
                            egui::Label::new(
                                egui::RichText::new(toast.text.as_str())
                                    .color(color)
                                    .strong(),
                            )
                            .selectable(false),
        ...
        match self.tab {
            ...
            Tab::Connections => {
                egui::CentralPanel::default()
                    .frame(pane_frame(ctx))
                    .show(ctx, |ui| {
                        // CentralPanel 不会自己滚动，连接多了下面就被裁掉，所以整页套一层滚动区
                        scroll_y(ui, |ui| self.connections(ui, ctx));
                    });
            }
            Tab::Logs => {
                egui::CentralPanel::default()
                    .frame(pane_frame(ctx))
                    .show(ctx, |ui| self.logs(ui, ctx));
            }
            Tab::About => {
                egui::CentralPanel::default()
                    .frame(pane_frame(ctx))
                    .show(ctx, |ui| {
                        scroll_y(ui, |ui| self.about(ui));
                    });
            }
        }
        if !self.started {
...
/// 内容面板底框：沿用窗口底色，只统一留白
fn pane_frame(ctx: &egui::Context) -> egui::Frame {
    egui::Frame::NONE
        .fill(ctx.style().visuals.panel_fill)
        .inner_margin(egui::Margin::symmetric(14, 10))
}

/// 纵向滚动区：`auto_shrink(false)` 让它始终占满可用高度，否则会被内容撑开、滚不起来
fn scroll_y(ui: &mut egui::Ui, add: impl FnOnce(&mut egui::Ui)) {
    egui::ScrollArea::vertical()
        .auto_shrink([false; 2])
        .show(ui, add);
}

/// 顶栏/底栏底框：比内容区略亮一档，并带一条描边作分隔
fn bar_frame(ctx: &egui::Context) -> egui::Frame {
    let visuals = ctx.style().visuals.clone();
    egui::Frame::NONE
        .fill(if visuals.dark_mode {
            egui::Color32::from_rgb(23, 29, 37)
        } else {
            egui::Color32::WHITE
        })
        .stroke(line(1.0, visuals.widgets.inactive.bg_stroke.color))
...
impl AronaGui {
    ...
    fn logs(&mut self, ui: &mut egui::Ui, _ctx: &egui::Context) {
        ...
        ui.horizontal(|ui| {
            section_title(ui, "实时日志");
            ui.add(
                egui::Label::new(
                    egui::RichText::new(format!("{} 行", self.log_lines.len()))
                        .size(12.0)
                        .weak(),
                )
                .selectable(false),
        ...
        let lines: Vec<&crate::runtime::log::LiveLine> = self.log_lines[start..]
            ...
            .collect();
        // 日志区单独一块「终端」底色的圆角面板：和页面其它部分拉开层次，长时间阅读时不刺眼
        let visuals = ui.visuals().clone();
        egui::Frame::NONE
...
impl AronaGui {
    ...
    fn about(&self, ui: &mut egui::Ui) {
        ...
        ui.add(
            egui::Label::new(
                egui::RichText::new(format!("Arona-rs v{}", env!("CARGO_PKG_VERSION")))
                    .strong()
                    .size(22.0)
                    .color(accent(dark)),
            )
...
/// wgpu 尝试变体：从高到低排，正常情况下第一个就够，后面几个是给环境刁钻的机器兜底的
///
/// 两个维度各有一档降级：着色器编译器先降（随包 DXC → 系统 FXC），后端范围再降
/// （DX12/Vulkan → 含 GL 的全部后端）。
#[derive(Clone, Copy, PartialEq, Eq)]
enum WgpuFlavor {
    /// DX12/Vulkan + 随程序附带的 DXC（最优：显卡渲染 + 最快最新的着色器编译器）
    Dxc,
    /// 所有后端（含 GL）+ 随程序附带的 DXC
    AllBackendsDxc,
    /// DX12/Vulkan + 系统自带的 FXC（d3dcompiler_47.dll，Win10+ 与服务器都有）
    Fxc,
    /// 所有后端 + 系统自带的 FXC：最后一搏，让 Vulkan/GL 也参与枚举
    AllBackendsFxc,
}

impl WgpuFlavor {
    /// 这一档用不用随包附带的 DXC
    fn uses_dxc(self) -> bool {
        matches!(self, Self::Dxc | Self::AllBackendsDxc)
    }

    /// 这一档要不要放开全部后端（含 GL）
    fn all_backends(self) -> bool {
        matches!(self, Self::AllBackendsDxc | Self::AllBackendsFxc)
    }
}
...
impl RendererAttempt {
    ...
    fn glow() -> RendererAttempt {
        RendererAttempt {
            ...
            // glow 不读 wgpu 配置，给个不影响结果的档位
            flavor: WgpuFlavor::Fxc,
            softgl: false,
        }
    }
...
fn has_hardware_opengl() -> bool {
    #[cfg(windows)]
    {
        ...
        let sub_key: Vec<u16> =
            OsStr::new(r"SOFTWARE\Microsoft\Windows NT\CurrentVersion\OpenGLDrivers")
                .encode_wide()
                .chain(Some(0))
                .collect();
        ...
        let mut key: isize = 0;
        let status =
            unsafe { RegOpenKeyExW(HKEY_LOCAL_MACHINE, sub_key.as_ptr(), 0, KEY_READ, &mut key) };
...
/// 渲染后端候选顺序：**从高到低**降级 —— 能用显卡就用显卡（且优先用随包的 DXC 编译
/// 着色器），实在不行才退到 CPU 软件 OpenGL(llvmpipe)，最后才是纯命令行模式。
///
...
///
/// `crash_fallback` 是崩溃看门狗的结论（[`guard_first_launch`]）：上一份进程被渲染后端
/// 直接干掉了，这次把 CPU 软件渲染提到最前面，别再在同一个坑里闪退。
fn renderer_candidates(args: &[String], crash_fallback: bool) -> Vec<RendererAttempt> {
    let requested =
        crate::find_arg(args, "--renderer=").or_else(|| std::env::var("ARONA_RENDERER").ok());
    candidates_for(
        requested.as_deref(),
        has_hardware_opengl(),
        crate::runtime::softgl::dir().is_some(),
        crate::runtime::softgl::requested(args) || crash_fallback,
        bundled_dxc_ready(),
    )
}
...
fn candidates_for(
    ...
    match requested {
        Some("wgpu") => wgpu_candidates(dxc_available),
        Some("glow") => vec![glow_candidate(
            has_hardware_gl,
            softgl_available,
            softgl_requested,
        )],
...
/// wgpu 的几种配置，按「从高到低」排：显卡 + 随包 DXC → 显卡 + 系统 FXC → 放开全部后端。
///
/// 没随包 DXC（未运行 scripts/fetch-dxc.ps1，或用户解压时漏了 dll）时直接跳过 DXC 档：
/// 那种机器上它只会白白多花一次实例创建的时间。
fn wgpu_candidates(dxc_available: bool) -> Vec<RendererAttempt> {
    let mut attempts = Vec::new();
    if dxc_available {
        attempts.push(RendererAttempt::wgpu(
            "wgpu(DX12/Vulkan+DXC)",
            WgpuFlavor::Dxc,
        ));
        attempts.push(RendererAttempt::wgpu(
            "wgpu(全部后端+DXC)",
            WgpuFlavor::AllBackendsDxc,
        ));
    }
    attempts.push(RendererAttempt::wgpu(
        "wgpu(DX12/Vulkan+FXC)",
        WgpuFlavor::Fxc,
    ));
    attempts.push(RendererAttempt::wgpu(
        "wgpu(全部后端+FXC)",
        WgpuFlavor::AllBackendsFxc,
    ));
    attempts
}
...
fn auto_candidates(
    ...
    attempts.extend(wgpu_candidates(dxc_available));
...
fn wgpu_options(flavor: WgpuFlavor) -> eframe::egui_wgpu::WgpuConfiguration {
    ...
    if let WgpuSetup::CreateNew(setup) = &mut config.wgpu_setup {
        setup.instance_descriptor.backends = if flavor.all_backends() {
            eframe::wgpu::Backends::all()
        } else {
            eframe::wgpu::Backends::PRIMARY
        };
        // 着色器编译器：绝不能用 `StaticDxc`——那要求 wgpu 打开 static-dxc 特性，而它链接的
        // 预编译 dxcompiler 静态库引用 ATL 符号(需要 atls.lib，见根 Cargo.toml 的说明)；
        // 特性没开时 wgpu-hal 会直接 panic，把整个兜底链打断。
        // 高优先档用**随包分发**的 DXC（安装包与便携 zip 都把 dxcompiler.dll/dxil.dll 放在
        // exe 同级，所以不需要装任何依赖）；这两档起不来时才降到系统自带的 FXC
        // (d3dcompiler_47.dll，Win10+ 与服务器上都有)。
        setup
            .instance_descriptor
            .backend_options
            .dx12
            .shader_compiler = if flavor.uses_dxc() {
            eframe::wgpu::Dx12Compiler::DynamicDxc {
                dxc_path: bundled_dxc_dll("dxcompiler.dll"),
                dxil_path: bundled_dxc_dll("dxil.dll"),
            }
        } else {
            eframe::wgpu::Dx12Compiler::Fxc
        };
...
/// exe 同级的某个文件（渲染运行库都平铺放在那里）
fn exe_sibling(file_name: &str) -> Option<std::path::PathBuf> {
    let path = std::env::current_exe().ok()?.parent()?.join(file_name);
    path.is_file().then_some(path)
}

/// 随包附带的 DXC 是否就位（dxcompiler.dll 与 dxil.dll 都在 exe 同级）
fn bundled_dxc_ready() -> bool {
    ["dxcompiler.dll", "dxil.dll"]
        .iter()
        .all(|name| exe_sibling(name).is_some())
}

/// 随程序附带的 dll 路径：exe 同级有同名文件时用绝对路径，否则交回裸文件名
/// （让系统按 DLL 搜索顺序去找，找不到就是找不到，绝不影响后面的 FXC 候选）
fn bundled_dxc_dll(file_name: &str) -> String {
    match exe_sibling(file_name) {
...
fn select_adapter(
    ...
    if adapters.is_empty() {
        ...
        return Err(
            "wgpu 没有枚举到任何适配器（DX12/Vulkan 后端创建失败，原因见上方 wgpu 日志）"
                .to_string(),
        );
    }
    ...
    if fallback_warning {
        crate::runtime::log::warning(
            "所有 wgpu 适配器都报告不支持当前窗口，仍尝试使用其中最优的一个",
        );
    }
    let info = adapter.get_info();
...
fn next_family_index(attempts: &[RendererAttempt], index: usize) -> Option<usize> {
    ...
    (index + 1..attempts.len())
        .find(|&other| is_wgpu_renderer(attempts[other].renderer) == want_wgpu)
}

/// 第 `index` 个候选卡死之后怎么办。
///
/// `handoff` 是「已经换过几次后端」，到 [`MAX_HANDOFFS`] 就老老实实退回命令行模式：
/// 否则 `--softgl`（列表 [软渲染, wgpu 若干档]）与强制 `--renderer=wgpu` 的场景可能
...
/// 崩溃看门狗的标记：子进程带着它启动，就不会再自己拉起一份（只盯一层，避免进程套娃）
const CRASH_GUARD_ENV: &str = "ARONA_GUI_GUARDED";

/// [`guard_first_launch`] 的结论
enum Guard {
    /// 盯的那一份已经正常退出：本进程直接收工，别再开第二份
    Done,
    /// 那一份被渲染后端整个干掉了（既没返回 Err 也没 panic，例如 access violation 闪退）：
    /// 本进程接着跑，但把 CPU 软件渲染提到降级链最前面
    RetrySoftgl,
    /// 不接管：本进程按原来的降级链自己跑
    Off,
}

/// 命令行里没有任何「用户自己做主」的痕迹时才挂看门狗（纯函数，便于单测）：
/// 显式指定过后端、已经在命令行模式、或者本来就是看门狗换后端重启的下一棒，都不插手。
fn guardable_args(args: &[String]) -> bool {
    args.iter().all(|arg| {
        arg != "--nogui"
            && !arg.starts_with("--renderer=")
            && !arg.starts_with("--gui-attempt=")
            && !arg.starts_with("--gui-handoff=")
    })
}

/// 要不要为这一次启动挂崩溃看门狗。本地没有软件 OpenGL 时插手也没意义（重试还是同一条链）。
fn should_guard(args: &[String], softgl_available: bool) -> bool {
    softgl_available
        && guardable_args(args)
        && std::env::var_os(CRASH_GUARD_ENV).is_none()
        && std::env::var("ARONA_RENDERER")
            .ok()
            .is_none_or(|value| value.trim().is_empty())
        && !crate::runtime::softgl::requested(args)
}

/// 另起一份进程去跑整条渲染降级链，本进程只等它的退出码。
///
/// 为什么需要：进程内的兜链只能处理「返回 Err」和「panic」两种情况，而显卡驱动 /
/// DX12 出问题时常见的是**直接把进程干掉**（闪退，什么都来不及写）——那样最好的那档
/// 一崩，后面所有兜底都没机会跑，用户看到的就是窗口一闪就没。
fn guard_first_launch(args: &[String]) -> Guard {
    let mut child =
        match crate::runtime::relaunch::spawn_self_child(args, &[], &[(CRASH_GUARD_ENV, "1")]) {
            Ok(child) => child,
            Err(err) => {
                crate::runtime::console::eprint_safe(&format!(
                    "[Arona] {err}；改由本进程直接启动管理面板"
                ));
                return Guard::Off;
            }
        };
    match child.wait() {
        Ok(status) if status.success() => Guard::Done,
        Ok(status) => {
            crate::runtime::log::error(format!(
                "管理面板进程异常退出（{status}），最可能是渲染后端把进程干掉了；\
                 本进程改用 CPU 软件渲染档重新启动"
            ));
            Guard::RetrySoftgl
        }
        // 连退出状态都拿不到：那份可能还在跑，本进程绝不能再起一个实例抢端口/数据库
        Err(_) => Guard::Done,
    }
}
...
pub fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let mut crash_fallback = false;
    if should_guard(&args, softgl_available) {
        match guard_first_launch(&args) {
            Guard::Done => return Ok(()),
            Guard::RetrySoftgl => crash_fallback = true,
            Guard::Off => {}
        }
    }
    let candidates = renderer_candidates(&args, crash_fallback);
    ...
    // 这里的尺寸是「逻辑点」，winit 会按屏幕缩放比换算成物理像素（150% 屏上开出 1800x1170）。
    // 换算成立的前提是 assets/arona.manifest 声明了 DPI 感知，别把它删掉。
    ...
    for ((index, attempt), failover) in candidates.into_iter().enumerate().zip(failovers).skip(skip)
    {
        let (tx, rx) = channel();
        ...
        // 渲染后端在初始化阶段 panic（wgpu 历史上出过）时不能带走整个进程：兜底链后面还有
        // 候选，机器人也已经在这个进程的 tokio 运行时里跑起来了。
        let attempt_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            eframe::run_native("Arona 管理面板", options, creator)
        }));
        finished.store(true, Ordering::SeqCst);
        let result = match attempt_result {
            Ok(Ok(())) => Ok(()),
            Ok(Err(err)) => Err(err.to_string()),
            Err(panic) => Err(format!("初始化时发生 panic: {}", panic_detail(&*panic))),
        };
        match result {
            Ok(()) => {
                outcome = Ok(());
            ...
            Err(reason) => {
                let message = format!("{} 后端启动失败: {reason}", attempt.name);
                crate::runtime::log::warning(&message);
                errors.push(message);
...
/// panic 载荷里的信息（`String`/`&str` 之外只能给出通用提示）
...
mod tests {
    ...
    /// 桌面场景：有硬件 OpenGL 时先用硬件 glow（最快），后面才是 wgpu 的各档，不掺软渲染
    #[test]
    fn auto_uses_hardware_opengl_first_on_desktop() {
        let attempts = candidates_for(None, true, true, false, true);
        assert_eq!(names(&attempts)[0], "glow(OpenGL)");
        assert!(!attempts[0].softgl);
        assert_eq!(attempts.len(), 5, "硬件 glow + wgpu 的四种配置");
        assert!(attempts.iter().all(|attempt| !attempt.softgl));
    }

    /// 随包 DXC 就位：降级链**从高到低** —— 显卡+DXC 打头，系统 FXC 两档在后，
    /// CPU 软件渲染排在最后（只在前面全都起不来/卡死时才用）
    #[test]
    fn ladder_goes_high_to_low_with_bundled_dxc() {
        let attempts = candidates_for(None, false, true, false, true);
        assert_eq!(
            names(&attempts),
            vec![
                "wgpu(DX12/Vulkan+DXC)",
                ...
                "wgpu(DX12/Vulkan+FXC)",
                "wgpu(全部后端+FXC)",
                "glow(软件 OpenGL llvmpipe)",
            ]
        );
        assert!(
            attempts.last().unwrap().softgl,
            "CPU 软渲染必须排在最后兜底"
        );
    }

    /// 没带 DXC（未运行 scripts/fetch-dxc.ps1，或解压时漏了 dll）：跳过 DXC 两档，
    /// 也别把 glow 硬件后端塞进去（没有 ICD 时它甚至可能直接把进程干掉）
    #[test]
    fn ladder_skips_dxc_tiers_when_not_bundled() {
        let attempts = candidates_for(None, false, false, false, false);
        assert_eq!(
            names(&attempts),
            vec!["wgpu(DX12/Vulkan+FXC)", "wgpu(全部后端+FXC)"]
        );
        assert!(attempts.iter().all(|attempt| !attempt.softgl));
    }
    ...
    fn requested_renderer_is_respected() {
        assert_eq!(
            names(&candidates_for(Some("wgpu"), true, true, false, true)).len(),
            4
        );
        assert_eq!(
            names(&candidates_for(Some("wgpu"), true, true, false, false)).len(),
            2
        );

        let softgl = candidates_for(Some("softgl"), true, false, false, true);
        assert_eq!(softgl.len(), 1);
        assert!(softgl[0].softgl);

        // --renderer=glow：没有 ICD 但带 softgl 时自动改用软渲染（否则必然失败）
        let glow = candidates_for(Some("glow"), false, true, false, true);
        assert_eq!(glow.len(), 1);
        assert!(glow[0].softgl);

        // 有硬件 OpenGL 时 --renderer=glow 就是硬件 glow
        let hardware = candidates_for(Some("glow"), true, true, false, true);
    ...
    fn softgl_request_always_goes_first() {
        let attempts = candidates_for(None, true, true, true, true);
        assert_eq!(names(&attempts)[0], "glow(软件 OpenGL llvmpipe)");
        assert!(attempts[0].softgl);
        assert_eq!(attempts.len(), 5);
    }

    /// 崩溃看门狗的适用范围：只在「什么都没指定」的首启动插手
    #[test]
    fn crash_guard_only_for_plain_launch() {
        let arg = |values: &[&str]| -> Vec<String> {
            values.iter().map(|value| value.to_string()).collect()
        };
        assert!(guardable_args(&arg(&["arona-rs.exe"])));
        // 用户自己做主 / 已经是命令行模式 / 看门狗重启的下一棒
        assert!(!guardable_args(&arg(&["arona-rs.exe", "--renderer=wgpu"])));
        assert!(!guardable_args(&arg(&["arona-rs.exe", "--nogui"])));
        assert!(!guardable_args(&arg(&["arona-rs.exe", "--gui-attempt=2"])));
        assert!(!guardable_args(&arg(&["arona-rs.exe", "--gui-handoff=1"])));
        // 没有软件 OpenGL 兜底时不插手：重试还是同一条链，白白多起一个进程
        assert!(!should_guard(&arg(&["arona-rs.exe"]), false));
    }

    /// 卡死换后端：wgpu 卡住时直接跳到另一族（软渲染），不用把 wgpu 几档配置各等一遍超时
    #[test]
    fn failover_jumps_across_renderer_families() {
        let no_icd = candidates_for(None, false, true, false, true);
        // [wgpu x4, 软渲染]：第一个 wgpu 卡死 -> 直接跳到软渲染
        assert_eq!(next_family_index(&no_icd, 0), Some(4));
        assert_eq!(next_family_index(&no_icd, 3), Some(4));
        // 软渲染卡死 -> 后面没有别的族了
        assert_eq!(next_family_index(&no_icd, 4), None);

        // [glow(硬件), wgpu x4]：硬件 glow 卡死 -> 跳到第一个 wgpu
        let desktop = candidates_for(None, true, true, false, true);
    ...
    fn failover_stops_after_max_handoffs() {
        let no_icd = candidates_for(None, false, true, false, true);
        assert!(matches!(
            failover_for(&no_icd, 0, true, 0),
            Failover::Attempt(4)
        ));
        assert!(matches!(
            failover_for(&no_icd, 0, true, MAX_HANDOFFS),
            Failover::NoGui
        ));
        // 软渲染自己卡死、后面又没有别的族 -> 命令行模式（不会回头再试 wgpu）
        assert!(matches!(failover_for(&no_icd, 4, true, 0), Failover::NoGui));

        // 强制 --renderer=wgpu（候选里没有软渲染）卡死时：转到软件 OpenGL 再试一次
        let forced = candidates_for(Some("wgpu"), true, true, false, false);
        assert!(matches!(
            failover_for(&forced, 0, true, 0),
            Failover::Softgl
        ));
        assert!(matches!(
            failover_for(&forced, 0, false, 0),
            Failover::NoGui
        ));
    }
    ...
    fn log_line_color_follows_theme() {
        ...
        assert_ne!(
            log_line_color("普通错误", true, false),
            egui::Color32::BLACK
        );
