...
pub fn spawn_self(
    ...
    build_command(args, extra, envs, remove_envs, strip_args, strip_prefixes)
        .and_then(spawn)
        .map(|_| ())
}

/// 同 [`spawn_self`]，但交回子进程句柄：GUI 崩溃看门狗要等它的退出码才能决定下一步。
pub fn spawn_self_child(
    args: &[String],
    extra: &[String],
    envs: &[(&str, &str)],
) -> Result<std::process::Child, String> {
    build_command(args, extra, envs, &[], &[], &[]).and_then(spawn)
}

fn spawn(mut command: Command) -> Result<std::process::Child, String> {
    let exe = command.get_program().to_string_lossy().into_owned();
    command
        .spawn()
        .map_err(|err| format!("拉起自身的副本失败({exe}): {err}"))
}
