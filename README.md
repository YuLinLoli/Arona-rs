...
本 README 只讲框架。仓库里带的碧蓝档案插件（`plugins/bluearchive`）是**当前在使用的那个功能包**，
它的命令、玩法、数据来源与配置项都不属于框架，本文一律不列 —— 想知道插件怎么写、能拿到哪些接口，
看 [PLUGIN_DEVELOPMENT.md](PLUGIN_DEVELOPMENT.md)。

## 框架与插件的分工

```
arona-host（宿主 exe）──▶ arona（框架库）◀── 功能插件（依赖框架，实现 AronaPlugin）
```
...
框架**不依赖任何插件**，插件单向依赖框架。构建时一个插件都不带，产物就是纯框架 exe：
连上 QQ 之后收得到消息，但没有任何命令会被响应（连 `/帮助` 都没有）。

## 能力细节

- **只做 OneBot 11**：正向 WebSocket / 反向 WebSocket / 正向 HTTP / 反向 HTTP 四种连接，
  同一类型可配多个实例，可直接对接 NapCat、Lagrange、LLOWeb 等实现端。
- **单文件可执行**：产物只有一个 exe，没有 JVM、没有 Mirai 控制台、没有需要用户摆放的插件目录；
  插件与框架同仓静态编译，功能包的增删只改 `plugins.toml` 与 host 的 `Cargo.toml`。
...
Releases 里有两种安装包，除了「带不带功能插件」没有区别：

| 文件 | 内容 |
| --- | --- |
| `arona-rs-<版本号>-setup-win-x64.exe` | 框架 + 功能插件（默认构建带的就是这一种） |
| `arona-rs-<版本号>-framework-setup-win-x64.exe` | 纯框架，不含任何功能插件，也不释放插件的默认配置 |
...
4. 从开始菜单启动「Arona-rs」打开管理面板：在「OneBot 连接」页加连接并「保存并热重载」，
...
还有两个**转交给插件**的开关，框架自己不用：`--test-notify` 会原样放进 `PluginContext::test_notify`，
由插件决定要不要安排一次自检；`ARONA_PLUGINS` 只在构建期生效，见下文「构建」。
...
<运行目录>/
  ...
  logs/                    按天滚动的日志
  plugins/
    <插件>/plugin.yml       插件清单（id / 名称 / 版本），登记阶段由框架写出
```
...
```bash
cargo build-release      # 发布构建(默认含管理 GUI 与功能插件), 产物: target/release/arona-rs[.exe]
...
这些是 `.cargo/config.toml` 里的别名，都固定打在 `-p arona-host` 上，也可以直接写全命令。

### 带不带插件

host 对每个功能插件开一个**同名 feature**（`default` 里全开），`build.rs` 按 `CARGO_FEATURE_*`
决定 `plugins.toml` 里的条目要不要生成注册代码 —— 关掉的档位**既不链接也不注册**。

```bash
# 纯框架 exe（不含任何功能插件）
cargo build --release -p arona-host --no-default-features --features gui

# 同一份东西打成安装包：产物名带 -framework，且不释放插件的默认配置
ARONA_PLUGINS=0 cargo installer

# 只带 GUI 与某一个插件
cargo build --release -p arona-host --no-default-features --features gui,bluearchive-plugin
```

`ARONA_PLUGINS=0`（也认 `none`/`false`/`off`）同样作用于 `cargo dist`。
...
- 包内含程序介绍（带插件是 [`installer/intro.txt`](installer/intro.txt)，纯框架是
  [`installer/intro-framework.txt`](installer/intro-framework.txt)，按 `/DWithPlugins` 切换）、
  AGPLv3 全文与中文译本、默认配置模板（[`installer/defaults/`](installer/defaults)，`onlyifdoesntexist` 释放）。
  `defaults/arona.yml` 里只有框架自己的项，插件的配置区由插件首次运行时自动补上，
  **安装包不需要跟着插件改**。
...
同一个脚本还负责读 `plugins.toml` 生成功能插件注册代码。换图标：
...
| 产物 | 说明 |
| --- | --- |
| `arona-rs-<版本号>-win-x64.zip` | 便携版（带功能插件）：`arona-rs.exe` + DXC 两个 dll + `softgl/` |
| `arona-rs-<版本号>-setup-win-x64.exe` | 安装包：介绍 + AGPLv3 全文 + 默认配置 + DXC（必装）+ 可选 `softgl/` |
| `arona-rs-<版本号>-framework-setup-win-x64.exe` | 纯框架安装包：同样的东西，但不链接任何功能插件、介绍页换成「这一版有什么/没什么」 |
...
```bash
cargo test --workspace                 # 离线单测（生命周期门控/配置迁移/钩子/隔离实例）
cargo test --release --workspace       # CI 口径
cargo clippy --workspace --all-targets # 框架侧 error 级必须清零
cargo fmt --all
cargo check -p hello-plugin            # 示例插件跟着框架接口一起编译
```

## 仓库结构

「框架 + 插件」的 Cargo 虚拟 workspace（`resolver = "2"`）：
...
crates/arona-host/       宿主可执行：产物 bin = arona-rs
  build.rs   把图标与清单编进 Windows 产物，并依 plugins.toml + feature 开关生成插件注册代码
  src/main.rs  register_plugins() -> arona::run(args)
plugins/bluearchive/     碧蓝档案功能插件（当前在使用的那个功能包）
plugins/hello/           插件开发示例：命令 / 事件钩子 / 出站钩子 / 类型化配置 / 每日任务
...
以 `arona` 为依赖（**务必 `default-features = false`**，别让 GUI 顺着依赖扩散）实现 `AronaPlugin`，
提供无参 `::new()`，然后把类型路径写进 `plugins.toml`、给 host 加一条同名 feature：
...
impl AronaPlugin for MyPlugin {
    fn meta(&self) -> PluginMeta {
        PluginMeta::new("my-plugin", "MyPlugin", "0.1.0", "我的功能")
    }
    fn install(&self, reg: &PluginRegistrar) -> Result<(), String> { Ok(()) }
    fn configure(&self, ctx: &PluginContext) -> Result<(), String> { Ok(()) }
}
```

命令怎么登记、事件怎么按子类订阅、配置区怎么声明、停用后框架回收哪些东西，逐条写在
...
- 群里发命令没人回应：先确认带没带功能插件（纯框架构建就是这么安静），再到「群管理」页看
